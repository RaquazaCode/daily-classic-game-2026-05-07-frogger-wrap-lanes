...
function readStateFromText(text) {
  return JSON.parse(text);
}

async function getState(page) {
  const text = await page.evaluate(() => window.render_game_to_text());
  return readStateFromText(text);
}

async function advanceUntil(page, predicate, { stepMs = 100, maxSteps = 180 } = {}) {
...
(async () => {
  ...
  try {
    ...
    const openingFrames = await captureFrames(page, 'opening-crossing', 10, 140);
    const wrappedState = await advanceUntil(page, (state) => state.wrapsUsed >= 1, {
      stepMs: 100,
      maxSteps: 120,
    });
    await page.screenshot({ path: path.join(outDir, 'shot-1.png'), fullPage: true });
    writeJson('state-1.json', wrappedState);

    const wrapFrames = await captureFrames(page, 'wrap-shortcut', 8, 120);
    const homeState = await advanceUntil(page, (state) => state.filledHomes.length >= 1, {
      stepMs: 100,
      maxSteps: 200,
    });
    await page.screenshot({ path: path.join(outDir, 'shot-2.png'), fullPage: true });
    writeJson('state-2.json', homeState);
    ...
    createGif(openingFrames, path.join(gifDir, 'clip-01-opening-crossing.gif'));
    createGif(wrapFrames, path.join(gifDir, 'clip-02-wrap-shortcut.gif'));

    const mixedPauseResetFrames = path.join(outDir, 'frames-pause-reset-mix');
    rmSync(mixedPauseResetFrames, { recursive: true, force: true });
    mkdirSync(mixedPauseResetFrames, { recursive: true });
    const frameSources = [
    ...
    frameSources.forEach((source, index) => {
      copyFileSync(source, path.join(mixedPauseResetFrames, `frame-${String(index).padStart(2, '0')}.png`));
    });
    createGif(mixedPauseResetFrames, path.join(gifDir, 'clip-03-pause-reset.gif'));
